...
# df = pd.read_json("../../data/data.json", lines=True, precise_float=True, orient='records')

file_path = '../../data/data.json'
try:
    df = pd.read_json(file_path, precise_float=True, orient='records')
    print("JSON carregado com sucesso.")
except ValueError as e:
    print(f"Erro ao carregar JSON no pandas: {e}")
...
# df['reviews_amount'] = df['reviews_amount'].apply(lambda x: x.replace('(', '').replace(')', '') if isinstance(x, str) else x)
